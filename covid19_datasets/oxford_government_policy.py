...
def _load_dataset() -> pd.DataFrame:
    ...
    df = oxford_df[[c for c in oxford_df.columns if 'Notes' not in c and 'Flag' not in c and 'Unnamed' not in c]].drop(['Date', 'StringencyIndex', 'StringencyIndexForDisplay', 'M1_Wildcard', 'LegacyStringencyIndex', 'LegacyStringencyIndexForDisplay'], axis='columns')
